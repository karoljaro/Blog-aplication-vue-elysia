...
] satisfies Prisma.CategoryOnPostCreateManyInput[];


export default async function seedCreateCategoriesOnPost(tx: PrismaTx) {
    const client = tx ?? seedPrisma;

    try {
        await client.categoryOnPost.createManyAndReturn({
